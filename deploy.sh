...
scp -r -P 8421 ./docker-compose.yml $DEPLOY_SERVER_USER@$DEPLOY_SERVER_IP:~/
ssh -p 8421 $DEPLOY_SERVER_USER@$DEPLOY_SERVER_IP "docker login -u gitlab-ci-token -p ${CI_BUILD_TOKEN} ${CI_REGISTRY};"\
"docker-compose stop;"\
